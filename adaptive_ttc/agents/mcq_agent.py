...
from prompts.solve_mcq import (
    build_solve_mcq_prompt,
    build_reflection_mcq_prompt,
    build_decompose_mcq_prompt,
)
from config import MAX_DEPTH, MAX_WIDTH

LEAF_ERROR_TAG = "[LEAF_ERROR]"
ORIGINAL_TAG   = "[Original question for context]"
SUBTASK_TAG    = "[Your specific reasoning step to complete]"
...
class MCQAgent(BaseAgent):
    ...
    def run(self, task: str) -> str:
        print(f"\n{'  ' * self.depth}[MCQAgent depth={self.depth} | leaf={self.is_leaf}]")
        print(f"{'  ' * self.depth}  task: {task[:80]}{'...' if len(task) > 80 else ''}")

        # Step 1: 풀기
        answer = self.call(build_solve_mcq_prompt(task))
        print(f"{'  ' * self.depth}  → 초안: {answer[:80]}{'...' if len(answer) > 80 else ''}")

        # Step 2: Self-reflection
        reflection = self.call(build_reflection_mcq_prompt(task, answer))
        print(f"{'  ' * self.depth}  → reflection: {reflection[:80]}{'...' if len(reflection) > 80 else ''}")
        ...
        decompose_result = self.call(build_decompose_mcq_prompt(original, answer, reflection))
        ...
        for i, st in enumerate(sub_tasks[:self.max_width]):
            ...
            child = MCQAgent(depth=self.depth + 1, max_depth=self.max_depth, max_width=self.max_width)
            child_task = (
                f"{ORIGINAL_TAG}\n{original}\n\n"
                f"{SUBTASK_TAG}\n{st}"
            )
        ...
        print(f"{'  ' * self.depth}  ✓ merge 완료 → {final[:60]}")
        return final
    ...
    def _merge(self, original_task: str, sub_results: list) -> str:
        ...
        if not valid:
            ...
            messages = [
                ...
                },
                {
                    ...
                    "content": (
                        f"Question:\n{original_task}\n\n"
                        f"Failed attempts and error reasons:\n{error_text}\n\n"
                        "Now select the correct answer, avoiding the errors above."
                    )
                }
            ]
        ...
        valid_text = "\n".join(
            [f"Reasoning step {i+1}: {st}\nResult {i+1}: {ans}"
             for i, (st, ans) in enumerate(valid)]
        )
        user_content = (
            f"Question:\n{original_task}\n\n"
            f"Successful reasoning steps:\n{valid_text}"
        )
        if error_text:
            user_content += (
                f"\n\nFailed attempts (use as hints to avoid mistakes):\n{error_text}"
            )
        ...
        messages = [
            {
                ...
                "content": (
                    "You are a multiple choice solver. "
                    "You are given a question, successful reasoning steps, "
                    "and failed attempts with error reasons. "
                    "Use the successful steps and learn from the errors. "
                    "End your response with exactly: ANSWER: <A or B or C or D>"
                )
            },
            {
